...
namespace MoShan.Unity.EditorExpand
{
    ...
    internal sealed class MonoBehaviourButtonAttributeDrawer : Editor
    {
        #region 生命周期方法
        /// <summary>
        /// 绘制检视窗口 GUI 时
        ...
        private void DrawButton(string methodName, MethodInfo methodInfo)
        {
            ...
            if (GUILayout.Button(new GUIContent(methodName), GUILayout.ExpandWidth(true)))
            {
                ...
                foreach (UnityEngine.Object target in targets)
                {
                    ...
                    // 获取【调用方法的返回值】
                    object value = methodInfo.Invoke(currentTarget, new object[] { });
    ...
    internal sealed class ScriptableObjectButtonAttributeDrawer : Editor
    {
        #region 生命周期方法
        /// <summary>
        /// 绘制检视窗口 GUI 时
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (base.target.GetType().IsSubclassOf(typeof(ScriptableObject)))
            {
        ...
        private void DrawButton(string methodName, MethodInfo methodInfo)
        {
            ...
            if (GUILayout.Button(new GUIContent(methodName), GUILayout.ExpandWidth(true)))
            {
                ...
                foreach (UnityEngine.Object target in targets)
                {
                    ...
                    if (target.GetType().IsSubclassOf(typeof(ScriptableObject)))
                    {
                    ...
                    {
                        Debug.Log($"{methodInfo.Name}() 方法调用结果为空值");
                    }
                }
            }
        }

...
namespace MoShan.Unity.EngineExpand
{
    ...
    internal static class DrawGUIKnobUtility
    {
        ...
        /// 旋钮哈希值
        /// </summary>
        private static readonly int KNOB_HASH = nameof(DrawGUIKnobUtility).GetHashCode();
        #endregion
        ...
        /// 热旋钮角度
        /// </summary>
        private static float s_HotKnobAngle = 0.0f;
        #endregion
        ...
        public static float DrawKnob(Rect position, float knobRadius, float angle, bool isRoundToInt, bool isRetrunImmediately)
        {
            ...
            // 获取【控件编号】
            int controlId = GUIUtility.GetControlID(KNOB_HASH, FocusType.Passive, position);
            ...
            // 判断 <【当前控件】是否为【当前热控件】>、<【当前事件类型】是否为【拖拽鼠标】>
            if (isHasFocus)
            {
                // 设置【当前角度】为【热旋钮角度】
                currentAngle = s_HotKnobAngle;
            }
            else
            {
                // 设置【当前角度】为【输入角度】
            ...
            // 获取【旋钮位置】
            Rect knobPosition = new Rect(knobCenter - Vector2.one * knobRadius, 2 * knobRadius * Vector2.one);
            #endregion
            ...
            if (backgroundRadius > 32)
            {
                 ...
                 * 即默认情况下，若需要保证可视化效果，绘制区域的最小宽高为【42 px】。
                 */
